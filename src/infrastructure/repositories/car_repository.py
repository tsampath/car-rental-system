...
class CarRepository(BaseRepository[CarModel, CarEntity], ICarRepository):
    ...
    def get_by_make(self, make: str) -> CarEntity:
        ...
        record = self.session.query(CarModel).filter_by(make=make).first()
        return self.to_entity(record) if record else None
    
    def get_by_model(self, model: str) -> CarEntity:
        """Retrieve a Car by make."""
        record = self.session.query(CarModel).filter_by(model=model).first()
        return self.to_entity(record) if record else None
        
    def get_by_year(self, year: str) -> CarEntity:
        """Retrieve a Car by make."""
        record = self.session.query(CarModel).filter_by(year=year).first()
        return self.to_entity(record) if record else None
